...
void WriteDatatoHT1621(uint8_t Data, uint8_t num)
{
    ...
    for (i = 0; i < num; i++)
    {
        PinLow_WR; //拉低WR脚  Data线上的数据在WR脚位上升沿写入

        if (((Data & 0x80) >> 7) == 1) //如果当前bit为1，就拉高DATA引脚
            PinHigh_Data;
        if (((Data & 0x80) >> 7) == 0) //如果当前bit为0，就拉低DATA引脚
            PinLow_Data;

        PinHigh_WR; //当前这一bit写完，拉高WR脚，为下次WR为上升沿做准备
 ...
 * param	Addr：写入初始地址，Data：写入数据，Mode：0x80 /0xa0
 * return	无
 *******************************************************************************/
void WritetoHT1621(uint8_t Addr, uint8_t Data, uint8_t Mode)
{
    PinLow_CS;
    WriteDatatoHT1621(Mode, 3);      //写入0x80 /0xa0
